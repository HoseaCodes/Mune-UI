...
import { saveUserInfo } from '@/utils/apiCalls';
...
const Footer: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!validatePhoneNumber(phoneNumber, setError)) {
      return; 
    }
    saveUserInfo(e, setError);
  };
  ...
  return (
    <footer className="h-[673px] rounded-tl-[32px] md:rounded-tl-[48px] mt-[120px] rounded-tr-[32px] md:rounded-tr-[48px] p-10">
      <div className="flex justify-center items-center flex-col gap-8 pt-10 md:pt-24">
        ...
        <form className="hidden md:block" onSubmit={handleSubmit}>
          ...
          {error && <p className="text-violet-950 mt-2">{error}</p>}
        </form>
        <form className="w-[240px] md:hidden" onSubmit={handleSubmit}>
          <div className="border-2 bg-white px-2 border-neutral-300 flex justify-start gap-2 items-center rounded-[12px] h-[44px]">
            <Image width={0} height={0} src={WhatsappIcon} alt="" />
            <input
      ...
      <div className="md:grid grid-cols-2 mt-8 md:mt-28">
        <div className="flex flex-col gap-8 justify-start">
          <Image width={0} height={0} src={Logo} className="hidden md:block w-[190px]" alt="logo" />
            ...
            <div className="flex flex-col gap-6 mt-16 md:mt-0">
              ...
              <div className="flex items-center justify-start gap-3">
                <a
                  ...
                  className="text-[20px] text-stone-100 font-normal"
                >
                  <Image width={0} height={0} src={FbIcon} alt="" />
                </a>
                <a
                  href="#/"
                  className="text-[20px] text-stone-100 font-normal"
                >
                  <Image width={0} height={0} src={InstaIcon} alt="" />
                </a>
                <a
                  href="#/"
                  className="text-[20px] text-stone-100 font-normal"
                >
                  <Image width={0} height={0} src={TiktokIcon} alt="" />
                </a>
              </div>
            </div>
          </div>
