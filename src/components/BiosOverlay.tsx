...
const BioOverlay: React.FC<BioOverlayProps> = ({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-stone-950/75 z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-[90%] max-h-[90%] md:max-w-[800px] mx-auto relative">
        <button
          onClick={onClose}
          type="button"
          aria-label="close"
          className="absolute text-4xl top-2 right-2 text-gray-500"
        >
          &times;
        </button>
        <img
          ...
          className="block w-24 h-24 md:w-40 md:h-40 rounded-full shadow-md mb-4 mx-auto border-2 border-custom-bgreen"
        />
